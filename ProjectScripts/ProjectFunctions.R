...
GetCountMatrixHTseq <- function(countsDF, meta = Metadata, MetaSamleCol = "activemotif_id", countSampleRegEx = "^X", 
  ...
  HouseKeeping %<>% filter(!duplicated(.$PeakName), Mean > 100) 
...
GetCollapsedMatrix <- function(countsMatrixAnnot, collapseBy, FilterBy, meta = Metadata, normCol = NULL, title = NULL, samples = "All", CorMethod = "pearson", countSampleRegEx = "^X", MetaSamleCol = "activemotif_id", MetaSamleIDCol = "SampleID", groupCol = "condition"){
  ...
  annoRow = data.frame(Group = meta$Group,
                       Age = meta$Age,
                       Sex = meta$Sex,
                       RIN = meta$RIN,
                       row.names = meta[[MetaSamleIDCol]])
  annoCol = data.frame(Group = meta$Group,
...
GetCellularProportions <- function(Metadata, normCol = NULL, MetaSamplCol = "SampleID"){
  if("CellTypesATAC.tsv" %in% list.files("CellTypesATAC")){
    CellTypePeaks <- read.table("CellTypesATAC/CellTypesATAC.tsv", header = T, sep = "\t")
  } else {
    source(paste0("ProjectScripts/CellProportions.R"))
  }
  
  #Get relative cell proportion for the samples based on differential NeuN positive and negative cell ATACseq peaks  
  HTseqCountsSamples <- read.table(CellTypePeakCountLoc, header = T, sep = "\t")
  CellTypePeakData <- read.table("CellTypesATAC/data/dorsolateral_prefrontal_cortex.narrowPeak.gz", header = T, sep = "\t")
  names(CellTypePeakData) <- c("CHR", "START", "END", "PeakName", "Score", "Srand", "Enrichment", "pPvalue", "pQvalue", "OffsetFromStart")
  
  CellTypePeakData %<>% filter(pPvalue > 7) %>% .[!grepl("MT|GL|hs", .$CHR),]
  ...
  blackListedPeaks <- findOverlaps(query = CellTypePeakData %>% as(., "GRanges"), subject = BlackListed %>% as(., "GRanges"), maxgap = 0, type = "any", select = "all")
  
  CellTypePeakData <- CellTypePeakData[-queryHits(blackListedPeaks),]
  
  
  HTseqCountsSamples %<>% filter(Geneid %in% CellTypePeakData$PeakName)
  ...
  HTseqCountsSamplesOrg <- HTseqCountsSamples
  
  #Merge the replicates
  HTseqCountsSamples <- sapply(Metadata$SampleID, function(subj){
    Rep <- Metadata %>% filter(SampleID == subj) %>% select(Rep1, Rep2) %>% unlist %>% as.character()
    HTseqCountsSamples %>% select(Rep) %>% apply(1, sum)
  })
  
  HTseqCountsSamples <- cbind(HTseqCountsSamplesOrg %>% select(-matches("Sample")), HTseqCountsSamples)
  ...
  counMatrixSamples2b <- counMatrixSamples2[,grepl("A", colnames(counMatrixSamples2))]
  ...
  CellTypeSpecificPeaks$NeuNall <- CellTypePeaks %>% filter(log2FoldChange > 2, !duplicated(Peak_Gene)) %>% select(PeakName, symbol, log2FoldChange, padj) %>% arrange(padj)
  ...
  counMatrixSamples2 <- counMatrixSamples2[rownames(counMatrixSamples2) %in% (CellTypePeaks %>% filter(abs(log2FoldChange) > 2, !duplicated(PeakName)) %>% .$PeakName),]
